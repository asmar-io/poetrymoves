...
module poetrymoves::words2words{
  ...
  /**
  * @description make a poem object from a given set of word objects then burning the words objects 
  * @param words:      the vector of words objects used to make a poem and will be burned, eg: [0x1870562b595cf90d3b8419e51ebd8c5e72d006b5838fc2eb1654a3303ec3efdc,0x4f5bdf44951f26b89426f50744e1552244ccf08741662086349ee90d47f13e9e]
  * @param image_url:  the image_url of a certain poem, eg: https://gateway.pinata.cloud/ipfs/QmasgcJ9pmRbxn7LZs1zFsSzJS6pt6cVcAMfjK371uvzgr
  * @param background: the background of a certain poem, eg: blank
  * @param title:      the background of a certain poem, eg: "My First Poem"
  ...
  */
  ...
  /**
  * @description mint a set of words using the a certian pack configurations
  * @param mintTo: the address to mint the words objects to, eg: 0x5f16803bfb61017ffa523dc3bb81e92b64e6b43a8d705203631e10f8daa24e25
  * @param pack_name: the name of the pack to be minted, eg: 0xeb16803bfb61017ffa523dc3bb81e92b64e6b43a8d705203631e10f8daa24e25
  ...
  */
  ...
  public entry fun add_special_pack(pack_name: vector<u8>,price: u64,background_image: vector<u8>, pack_pos_quantity: vector<u64>, pack_parts: vector<vector<u8>>,wordsdata: &mut WordsData,ctx: &mut TxContext){
    assert!(wordsdata.admin == tx_context::sender(ctx),ENotOwner);
    
    let words = vec_map::empty<String,vector<String>>();
   
    let parts_of_speech = vec_map::empty();
    let i = 0;
    while(i < vector::length(&pack_pos_quantity)){
  ...
  public entry fun add_special_pack_pos(pack_name: vector<u8>, part_of_speech: vector<u8>, words: vector<vector<u8>>,wordsdata: &mut WordsData,ctx: &mut TxContext){
    assert!(wordsdata.admin == tx_context::sender(ctx),ENotOwner);
    ...
    let pack_config = df::borrow_mut<String,Pack>(&mut wordsdata.id,utf8(pack_name));
  ...
  public entry fun mutate_beneficiary(new_beneficiary: address,wordsdata: &mut WordsData,ctx: &mut TxContext){
      assert!(wordsdata.admin == tx_context::sender(ctx) ,ENotOwner);
  ...
  fun internal_mint_pack(pack_name: vector<u8>,kiosk: &mut Kiosk, cap: &KioskOwnerCap,wordsdata: &mut WordsData,coin: Coin<SUI>,ctx: &mut TxContext){
    ...
    while(i < vector::length(&PARTS_OF_SPEECH)){
      let part_of_speech = *vector::borrow(&PARTS_OF_SPEECH,i);
      let base_part_of_speech = *vector::borrow(&BASE_PARTS_OF_SPEECH,i);
  ...
  fun internal_mint_special_pack(pack_name: vector<u8>,kiosk: &mut Kiosk, cap: &KioskOwnerCap,wordsdata: &mut WordsData,coin: Coin<SUI>,ctx: &mut TxContext){
    ...
    while(i < vector::length(&keys)){
      ...
      let part_of_speech_count = vec_map::get(&pack_config.parts_of_speech,part_of_speech);
  ...
  module sui::words_test {
    ...
    fun mint_words() {
        ...
        ts::next_tx(&mut scenario, addr1);
        {
           ...
           words2words::add_special_pack(b"CRYPTO PACK",1_000_000_000,b"000",vector[3,1,0,0],vector[b"Noun Degen",b"Noun Coin/Chain",b"ADVERB",b"SUFFIX"],&mut wordsdata,ts::ctx(&mut scenario));
           words2words::add_special_pack_pos(b"CRYPTO PACK",b"Noun Degen", vector[b"air",b"ant",b"arm",b"art"],&mut wordsdata,ts::ctx(&mut scenario));
           words2words::add_special_pack_pos(b"CRYPTO PACK",b"Noun Coin/Chain", vector[b"Avalanche",b"Binance",b"Cardano",b"Chainlink"],&mut wordsdata,ts::ctx(&mut scenario));
